...
<div class="modal fade" id="beritaModal" tabindex="-1" aria-labelledby="beritaModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <form id="beritaForm" method="POST" action="<?php echo e(route('admin.berita.store')); ?>" enctype="multipart/form-data" novalidate>
                <?php echo csrf_field(); ?>
...
<script>
    ...
    document.getElementById('beritaForm').addEventListener('submit', function(e) {
        ...
        let url = isEditing 
            ? '<?php echo e(route("admin.berita.update", ":id")); ?>'.replace(':id', editId)
            : '<?php echo e(route("admin.berita.store")); ?>';

        console.log('Request URL:', url);
        console.log('Request method:', isEditing ? 'PUT' : 'POST');
        console.log('Store route name resolved to:', '<?php echo e(route("admin.berita.store")); ?>');
        ...
        btn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`<?php echo e(url('admin/berita/delete')); ?>/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json'
