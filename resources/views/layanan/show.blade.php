@extends('layouts.app')

@section('title', $service->title . ' - ' . ($companyInfo->company_name ?? 'Swabina Gatra'))

@section('meta')
<meta name="description" content="{{ Str::limit($service->description, 160) }}">
<meta name="keywords" content="{{ $service->title }}, layanan, swabina gatra">
<meta property="og:title" content="{{ $service->title }}">
<meta property="og:description" content="{{ Str::limit($service->description, 200) }}">
@if($service->image)
<meta property="og:image" content="{{ asset('storage/' . $service->image) }}">
@endif
@endsection

@section('content')
<!-- Hero Section -->
<section class="hero-section py-5" style="background: linear-gradient(135deg, #0056b3 0%, #003d82 100%); color: white;">
    <div class="container">
        <div class="row align-items-center min-vh-50">
            <div class="col-lg-8 mx-auto text-center">
                @if($service->icon)
                <div class="mb-4">
                    <i class="bi {{ $service->icon }} display-1"></i>
                </div>
                @endif
                <h1 class="display-4 fw-bold mb-3">{{ $service->title }}</h1>
                <p class="lead mb-4">{{ $service->subtitle }}</p>
                <a href="#details" class="btn btn-light btn-lg">
                    <i class="bi bi-arrow-down-circle me-2"></i>Pelajari Lebih Lanjut
                </a>
            </div>
        </div>
    </div>
</section>

<!-- Service Details Section -->
<section id="details" class="py-5 bg-light">
    <div class="container">
        <div class="row">
            <div class="col-lg-8 mx-auto">
                @if($service->image)
                <div class="mb-4">
                    <img 
                        src="{{ asset('storage/' . $service->image) }}" 
                        alt="{{ $service->title }}"
                        class="img-fluid rounded shadow-lg"
                        loading="lazy"
                        width="800"
                        height="450"
                    >
                </div>
                @endif

                <div class="card border-0 shadow-sm mb-4">
                    <div class="card-body p-4">
                        <h2 class="h3 mb-4">Tentang {{ $service->title }}</h2>
                        <div class="text-muted lh-lg">
                            {!! nl2br(e($service->description)) !!}
                        </div>
                    </div>
                </div>

                @if($service->features)
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        <h3 class="h4 mb-4">Fitur & Keunggulan</h3>
                        <div class="row g-3">
                            @foreach($service->features as $feature)
                            <div class="col-md-6">
                                <div class="d-flex align-items-start">
                                    <i class="bi bi-check-circle-fill text-success me-3 fs-5"></i>
                                    <span>{{ $feature }}</span>
                                </div>
                            </div>
                            @endforeach
                        </div>
                    </div>
                </div>
                @endif
            </div>
        </div>
    </div>
</section>

<!-- Related Services Section -->
@if($relatedServices->isNotEmpty())
<section class="py-5">
    <div class="container">
        <div class="text-center mb-5">
            <h2 class="h3">Layanan Lainnya</h2>
            <p class="text-muted">Jelajahi layanan kami yang lain</p>
        </div>
        <div class="row g-4">
            @foreach($relatedServices as $related)
            <div class="col-md-4">
                <div class="card h-100 border-0 shadow-sm hover-lift">
                    <div class="card-body text-center p-4">
                        @if($related->icon)
                        <div class="mb-3">
                            <i class="bi {{ $related->icon }} display-4 text-primary"></i>
                        </div>
                        @endif
                        <h3 class="h5 mb-3">{{ $related->title }}</h3>
                        <p class="text-muted small mb-4">{{ Str::limit($related->subtitle, 80) }}</p>
                        <a href="{{ route('layanan.show', $related->slug) }}" class="btn btn-outline-primary btn-sm">
                            Lihat Detail <i class="bi bi-arrow-right ms-1"></i>
                        </a>
                    </div>
                </div>
            </div>
            @endforeach
        </div>
    </div>
</section>
@endif

<!-- CTA Section -->
<section class="py-5 bg-primary text-white">
    <div class="container">
        <div class="row align-items-center">
            <div class="col-lg-8">
                <h2 class="h3 mb-2">Tertarik dengan {{ $service->title }}?</h2>
                <p class="mb-0">Hubungi kami untuk informasi lebih lanjut dan konsultasi gratis</p>
            </div>
            <div class="col-lg-4 text-lg-end mt-3 mt-lg-0">
                <a href="{{ route('kontakkami') }}" class="btn btn-light btn-lg">
                    <i class="bi bi-envelope me-2"></i>Hubungi Kami
                </a>
            </div>
        </div>
    </div>
</section>
@endsection

@push('styles')
<style>
    .min-vh-50 {
        min-height: 50vh;
    }
    
    .hover-lift {
        transition: transform 0.3s ease, box-shadow 0.3s ease;
    }
    
    .hover-lift:hover {
        transform: translateY(-10px);
        box-shadow: 0 1rem 3rem rgba(0, 0, 0, 0.175) !important;
    }
    
    .hero-section {
        position: relative;
        overflow: hidden;
    }
    
    .hero-section::before {
        content: '';
        position: absolute;
        top: 0;
        left: 0;
        right: 0;
        bottom: 0;
        background: url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='0.05'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E");
        opacity: 0.1;
    }
</style>
@endpush
