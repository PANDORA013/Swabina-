...
@section('page-title', 'Sekilas Perusahaan')

@section('breadcrumb')
    <li class="breadcrumb-item active" aria-current="page">Sekilas Perusahaan</li>
@endsection
    ...
    <div class="card shadow-sm">
        <div class="card-body">
            <div class="row">
                @if($sekilas->image)
                <div class="col-md-4">
                    <img src="{{ asset('storage/' . $sekilas->image) }}" 
                         class="img-fluid rounded" 
                         alt="Sekilas Perusahaan" />
                </div>
                @endif
                <div class="col-md-8">
                    <h4>{{ $sekilas->judul ?? 'Sekilas Perusahaan' }}</h4>
                    <p class="text-muted">{{ $sekilas->deskripsi ?? 'Belum ada deskripsi' }}</p>
                    
                    @if($sekilas->tahun_berdiri)
                    <div class="mt-3">
                        <strong>Tahun Berdiri:</strong> {{ $sekilas->tahun_berdiri }}
                    </div>
                    @endif
                    
                    @if($sekilas->jumlah_karyawan)
                    <div class="mt-2">
                        <strong>Jumlah Karyawan:</strong> {{ $sekilas->jumlah_karyawan }}
                    </div>
                    @endif
                </div>
            </div>
        ...
        <div class="modal-content">
            <form id="sekilasForm" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <input type="hidden" id="id" name="id" value="{{ $sekilas->id ?? '' }}">
                    
                    <!-- Judul -->
                    <div class="mb-3">
                        <label for="judul" class="form-label fw-bold">Judul <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="judul" name="judul" 
                               placeholder="Contoh: Sekilas Perusahaan" value="{{ $sekilas->judul ?? '' }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="deskripsi" name="deskripsi" rows="5" 
                                  placeholder="Masukkan deskripsi sekilas perusahaan..." required>{{ $sekilas->deskripsi ?? '' }}</textarea>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="tahun_berdiri" name="tahun_berdiri" 
                               placeholder="Contoh: 2010" min="1900" max="2100" value="{{ $sekilas->tahun_berdiri ?? '' }}">
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="jumlah_karyawan" name="jumlah_karyawan" 
                               placeholder="Contoh: 150" min="0" value="{{ $sekilas->jumlah_karyawan ?? '' }}">
                    </div>
...
<script>
    ...
    document.getElementById('editSekilasBtn').addEventListener('click', function() {
        ...
        @if($sekilas)
            document.getElementById('id').value = '{{ $sekilas->id }}';
            document.getElementById('judul').value = '{{ $sekilas->judul }}';
            document.getElementById('deskripsi').value = '{{ $sekilas->deskripsi }}';
            document.getElementById('tahun_berdiri').value = '{{ $sekilas->tahun_berdiri }}';
            document.getElementById('jumlah_karyawan').value = '{{ $sekilas->jumlah_karyawan }}';
        @endif
    ...
    document.getElementById('sekilasForm').addEventListener('submit', function(e) {
        ...
        let url = id 
            ? `{{ route('admin.sekilas.update', '') }}/${id}`
            : '{{ route('admin.sekilas.store') }}';
