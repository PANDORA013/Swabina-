                ...
                <table class="table table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <strong>{{ isset($item->title['id']) ? $item->title['id'] : 'N/A' }}</strong>
                            </td>
                            <td>
                                <span class="text-muted">
                                    {{ isset($item->description['id']) ? Str::limit($item->description['id'], 80) : 'N/A' }}
                                </span>
                            </td>
                            <td>
                            ...
                            <td>
                                <button class="btn btn-sm btn-warning editBtn me-1" 
                                    ...
                                    data-title-en="{{ isset($item->title['en']) ? $item->title['en'] : '' }}"
                                    data-description-id="{{ isset($item->description['id']) ? $item->description['id'] : '' }}"
                                    data-description-en="{{ isset($item->description['en']) ? $item->description['en'] : '' }}"
...
<script>
    ...
    document.getElementById('beritaForm').addEventListener('submit', function(e) {
        ...
        let url = isEditing 
            ? `{{ route('admin.berita.update', '') }}/${editId}`
        ...
        btn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch(`{{ route('admin.berita.destroy', '') }}/${id}`, {
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                            'Accept': 'application/json'
